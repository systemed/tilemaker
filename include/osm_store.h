...
using mmap_linestring_t = boost::container::vector<Point,  boost::interprocess::node_allocator<Point, mmap_file_t::segment_manager>>;
BOOST_GEOMETRY_REGISTER_LINESTRING(mmap_linestring_t)
...
class OSMStore
{
    ...
    using linestring_t = mmap_linestring_t;
    ...
    using multi_linestring_t = boost::container::vector<linestring_t, scoped_alloc_t<linestring_allocator_t>>;
    using multi_linestring_allocator_t = boost::interprocess::node_allocator<multi_linestring_t, mmap_file_t::segment_manager>;
    using multi_linestring_store_t = std::deque<multi_linestring_t, scoped_alloc_t<multi_linestring_allocator_t>>;
    multi_linestring_store_t *generated_multi_linestring_store;

    using ring_t = linestring_t;
    using ring_allocator_t = linestring_allocator_t;
    using polygon_t = std::pair<ring_t, std::deque<ring_t, scoped_alloc_t<ring_allocator_t>>>;
    using polygon_allocator_t = boost::interprocess::node_allocator<polygon_t, mmap_file_t::segment_manager>;
    using polygon_store_t = std::deque<polygon_t, scoped_alloc_t<polygon_allocator_t>>;
    polygon_store_t *generated_polygon_store;

    using multi_polygon_store_t = std::deque<std::pair<std::size_t, std::size_t>, 
          boost::interprocess::node_allocator<std::pair<std::size_t, std::size_t>, mmap_file_t::segment_manager>>;
    ...
    void reopen() {
        ...
        generated_multi_linestring_store = mmap_file.find_or_construct<multi_linestring_store_t>
            ("multi_linestring_store")(mmap_file.get_segment_manager());
        generated_polygon_store = mmap_file.find_or_construct<polygon_store_t>
            ("polygon_store")(mmap_file.get_segment_manager()); 
...
public:
    ...
    template<typename Iterator>
    std::size_t store_linestring(Iterator begin, Iterator end)
    {
        perform_mmap_operation([&]() {
            generated_linestring_store->resize(generated_linestring_store->size() + 1);
            generated_linestring_store->back().resize(std::distance(begin, end));
            std::copy(begin, end, generated_linestring_store->back().begin());
        });
        return generated_linestring_store->size() - 1;
    }

    mmap_linestring_t retrieve_linestring(std::size_t index) const {
        return generated_linestring_store->at(index);
    }

    template<typename Iterator>
    std::size_t store_multi_linestring(Iterator begin, Iterator end)
    {
        perform_mmap_operation([&]() {
            generated_multi_linestring_store->resize(generated_multi_linestring_store->size() + 1);            
            for(auto i = begin; i != end; ++i) {
                generated_multi_linestring_store->back().resize(i->size());
                std::copy(i->begin(), i->end(), 
                        generated_multi_linestring_store->back().at(std::distance(begin, i)).begin());
            }
        });
        return generated_multi_linestring_store->size() - 1;
    }

    template<typename T>
    std::size_t store_polygon(T const& polygon)
    {
        perform_mmap_operation([&]() {
            generated_polygon_store->resize(generated_polygon_store->size() + 1);

            // Copy the outer ring
            generated_polygon_store->back().first.resize(polygon.outer().size());
            std::copy(polygon.outer().begin(), polygon.outer().end(), generated_polygon_store->back().first.begin());

            // Store the inner rings
            generated_polygon_store->back().second.resize(polygon.inners().size());
            for(std::size_t i = 0; i < polygon.inners().size(); ++i) {
                generated_polygon_store->back().second[i].resize(polygon.inners()[i].size());
                std::copy(polygon.inners()[i].begin(), polygon.inners()[i].end(), generated_polygon_store->back().second[i].begin());
            }
        });
        return generated_polygon_store->size() - 1;
    }

    Polygon retrieve_polygon(std::size_t index) const {
        Polygon result;
        auto const &polygon = generated_polygon_store->at(index);
        
        result.outer() = Polygon::ring_type(polygon.first.begin(), polygon.first.end());

        auto &inners = result.inners();
        inners.resize(polygon.second.size());
        for(std::size_t i = 0; i < polygon.second.size(); ++i) {
            inners[i] = Polygon::ring_type(polygon.second[i].begin(), polygon.second[i].end());
        }
    ...
    template<typename Iterator>
    std::size_t store_multi_polygon(Iterator begin, Iterator end)
    {
         perform_mmap_operation([&]() {
            generated_multi_polygon_store->resize(generated_multi_polygon_store->size() + 1);            
            generated_multi_polygon_store->back().first = generated_polygon_store->size();     
            for(auto i = begin; i != end; ++i) {
                store_polygon(*i);
            }
            generated_multi_polygon_store->back().second = generated_polygon_store->size();     
        });
        return generated_multi_polygon_store->size() - 1; 
    }

    MultiPolygon retrieve_multi_polygon(std::size_t index) const {
        MultiPolygon result;
        auto const &mp = generated_multi_polygon_store->at(index);
        result.resize(mp.second - mp.first);
        for(std::size_t i = 0; i < result.size(); ++i) {
            result[i] = retrieve_polygon(mp.first + i);
        }
