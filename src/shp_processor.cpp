...
void ShpProcessor::read(class LayerDef &layer, uint layerNum)
{
	...
	for (int i=0; i<numEntities; i++) {
		...
		boost::asio::post(pool, [&, i, shape]() {
			...
			int32_t score = 0;
			AttributeIndex attrIdx = readShapefileAttributes(dbf, i, columnMap, columnTypeMap, layer, layer.minzoom, score);
			// process geometry
			processShapeGeometry(shape, attrIdx, layer, layerNum, hasName, name, score);
			SHPDestroyObject(shape);
		});
	}
	pool.join();
